...
class BackwardSlicing(object):
    def __init__(self, db, file, buggy_line_num,file_path):
        self.db = db
        self.file = file
    ...
    def get_related_var_lines(self, line_stack, analyzed_lines, show_use_by=False):
        ...
        while line_stack:
            ...
            if len(analyzed_lines) > CONTEXT_WINDOW_LENGTH:
                print("exit as context limit has reached")
        ...
        for var in variables:
            ...
            for reference in var.refs():
                ...
                if current_line in checked_lines:
                    continue
                if var.parent():
                    condition =  current_line > self.method_start_line_num and current_line not in analyzed_lines and var.parent().name() != "System" and reference.kind().longname() in REL_KIND
                else:
                    condition =  current_line > self.method_start_line_num and current_line not in analyzed_lines  and reference.kind().longname() in REL_KIND
                if condition:
...
def process_single_line(project_path, file_name, line_number):
  ...
  if file:
     pass
      ...
      db.close()
      return [], []
  bs = BackwardSlicing(db, file, buggy_line_num,file.longname())
  comment_lines = bs.comment_lines
  context_lines, line_to_col_map = bs.run()
  context_lines = [line for line in context_lines if line not in comment_lines]
  statements = []
...
def process_multiple_lines(project_path, file_name, line_numbers):
    ...
    for line in buggy_lines:
      if line in multi_context_lines:
         continue
      bs = BackwardSlicing(db, file, line,file.longname())
      comment_lines = bs.comment_lines
      context_lines, line_to_col_map = bs.run()
    ...
    multi_context_lines = [line for line in multi_context_lines if line not in comment_lines]  
...
# use following if dual slicing for a file is needed
